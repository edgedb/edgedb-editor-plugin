...
def main():
    ...
    eql_base_path = (
        pathlib.Path(__file__).resolve().parent.parent / 'grammars' / 'src'
    )

    dunder_builtins = [kw for kw in meta.EdgeQL.reserved_keywords
                       if kw[:2] == '__']
    fn_builtins = sorted(meta.EdgeQL.fn_builtins)
    constraint_builtins = sorted(meta.EdgeQL.constraint_builtins)
    type_builtins = sorted(set(meta.EdgeQL.type_builtins) | {'anytype'})
    ...
    reserved_keywords = sorted(
      set(meta.EdgeQL.reserved_keywords)
      # special keywords that have other meaning
      - set(dunder_builtins)
      - set(type_builtins)
    )
